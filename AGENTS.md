...
- Simplicité des publications : réutiliser une preuve TEST encore valide, sans republier TEST à chaque demande PROD. L'assistant gère les références techniques et présente un bilan court. La cible approuvée est une seule confirmation GitHub avant l'écriture PROD ; tant que l'adaptation décrite dans `docs/releases/PROCEDURE.md` n'est pas réalisée, respecter toutes les protections existantes. Une modification documentaire ne rend pas cette simplification opérationnelle.
