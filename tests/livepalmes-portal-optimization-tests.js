...
assert.ok(portal.includes('data-engagement-club-relay-compose="${escapeHtml(relayId)}"'));
assert.ok(portal.includes("await saveEngagementClubRelays(null, elements.engagementsClubRelayDialogMessage)"));
...
assert.ok(portal.includes("function engagementRelayMemberFullSummary"));
...
assert.ok(portal.includes('addEventListener("pointerdown"'));
