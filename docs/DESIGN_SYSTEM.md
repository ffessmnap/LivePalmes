...
Le turquoise constitue la couleur de marque principale. Les en-têtes publics et Performances utilisent un dégradé sombre vers turquoise, complété par un motif de vagues. L’accueil, le calendrier et les pages publiques Performances partagent une largeur maximale de 1 120 px ; les bandeaux Performances sont centrés, bordés et alignés sur leurs filtres et données. Les logos LivePalmes et FFESSM Nage avec palmes assurent l’identification institutionnelle.
...
| Public compétition | `styles.css`, `public.css` | `index.html`, `resultats.html`, `series-public.html`, `medailles.html` |
...
Le portail utilise une largeur fluide plafonnée à `1720 px`, avec des gouttières latérales d’au moins `16 px`. Son en-tête blanc, compact et collant sépare clairement la marque, le club et le compte : logo fédéral et « Portail LivePalmes » à gauche, code court du club dans une capsule centrale, puis menu et compte à droite. Le code du club est enrichi du nom complet au survol ; le niveau régional ou national reste dans « Mon compte », sans encombrer le bandeau. Pour une personne habilitée au changement national de club, cette capsule devient un bouton ouvrant une fenêtre de recherche. Le changement est signalé comme temporaire à la session et le retour au club d’appartenance reste explicite. Sous `680 px`, l’ordre devient bouton de navigation, logo fédéral, « LivePalmes », puis code du club accompagné d’une icône de profil et ouvrant le menu du compte. Le bouton de navigation utilise une surface turquoise claire sans bordure visible au repos et ses trois lignes deviennent une croix lorsque le menu est ouvert ; le code est résolu depuis le référentiel statique des clubs, sans lecture Firebase supplémentaire. Le bandeau introductif « Portail LivePalmes / Vue d’ensemble » est masqué uniquement sur mobile afin que les cartes commencent directement sous l’en-tête. Pendant la restauration de la session, un indicateur neutre masque simultanément le formulaire de connexion et le contenu protégé ; la connexion ne s’affiche qu’après confirmation d’une session absente. La hauteur reste bornée à environ `54 px` sur mobile et `60 px` sur écran plus large.
...
Le contenu d’une fiche compétition suit une même structure pour tous ses onglets : un résumé ou une barre de contexte, puis des sections blanches bordées et enfin les actions. Sa marge dans la carte et le retrait du panneau actif consomment les gouttières responsives communes ; ils ne recréent pas deux espacements fixes de `16 px` et `14 px` sur les écrans étroits. L’ajout rapide au calendrier demande la ville, sans la recopier comme équipement ou site ; l’adresse et l’équipement/site restent facultatifs dans la fiche détaillée. En modification organisateur, le formulaire Général place les libellés au-dessus des champs dans une grille compacte : deux colonnes à partir de `701 px`, puis trois colonnes pour Compétition et Paramètres sportifs à partir de `1180 px`. Ville, adresse sans ville et équipement/site partagent une même ligne dans cet ordre sur grand écran ; les régions invitées et la présentation publique occupent toute la largeur. Engagements et Contacts de la compétition restent sur deux colonnes. Sous `701 px`, tous les champs repassent sur une colonne et conservent leur hauteur tactile. Le choix d’annulation reste une option compacte et les actions d’enregistrement sont répétées dans une barre en fin de formulaire. Les frais appartiennent à la section Engagements : leurs champs utilisent deux colonnes sur tablette, quatre sur grand écran et une sur mobile, puis sont enregistrés avec l’ensemble de la fiche. Côté Club, les mêmes informations sont consultées directement dans Général. Le nom de l’organisateur est regroupé avec les trois emails dans Contacts de la compétition. Ces emails sont uniquement informatifs et ne déclenchent aucun envoi automatique. Les documents utilisent des cartes compactes avec un badge d’état. En lecture seule, les valeurs restent visuellement distinctes sans masquer l’état désactivé des champs.

Dans la fenêtre d’un événement passé du calendrier public, la consultation ne reprend pas l’apparence d’un formulaire désactivé : seules les valeurs renseignées sont présentées dans une grille libellé-valeur sur deux colonnes, puis une colonne sous `600 px`. Les dates et localisations identiques sont dédupliquées, le programme vide est masqué et les statuts utilisent des badges. La GED reste modifiable mais son formulaire est replié sous la liste des documents ; une fois ouvert, Fichier et Description occupent toute la largeur tandis que Titre et Catégorie partagent la ligne sur ordinateur. Dans la liste, les titres et noms de fichiers longs reviennent à la ligne dans une colonne extensible ; les actions conservent une colonne droite stable et ne passent sous le document que sous `600 px`. La fenêtre est limitée à `680 px`, masque les zones de message vides et se ferme avec la croix placée en haut à droite, la touche Échap ou un clic hors de la carte.

Le programme utilise des sections repliables, avec un fond turquoise léger pour la session active ; ses groupes restent en deux colonnes jusqu’à `520 px`, puis passent en une colonne sur les plus petits écrans. Dans l’en-tête d’une session, « Session », « Date » et « Début » restent toujours alignés sur trois colonnes, y compris sur mobile, tandis que les éventuelles actions passent sur la ligne suivante. Chaque course tient sur une seule ligne : en piscine, son premier passage conserve le marqueur de sexe et un bouton compact dont la largeur suit la phase affichée ; son menu flottant propose Course directe, Séries + finale(s) et Séries lentes / série rapide sans élargir la ligne. La ligne montre seulement la phase qui y est nagée et le second passage affiche automatiquement Finale(s) ou Série rapide sans sélecteur indépendant. En Eau libre, les courses directes n’affichent aucun format ; seul le 150 m élimination propose Course directe ou Séries + finale(s). Les marqueurs féminin, masculin, mixte et combiné conservent leurs couleurs métier.

Dans le formulaire de modification organisateur, les frais sont intégrés visuellement à la section « Engagements » et sont enregistrés avec le reste de la fiche. Le choix « Aucun frais d’engagement » reste une simple ligne sans carte et masque immédiatement tous les champs tarifaires lorsqu’il est coché. Les trois montants et le lien de paiement sont présentés sans sous-cartes : quatre colonnes dès que la largeur le permet, deux sur les formats intermédiaires et une sur mobile. La phrase fixe sur l’échéance de paiement et le surplus de `50 €` n’y est pas affichée ; les règles métier enregistrées restent inchangées.
...
Dans une fiche compétition, aucun bandeau permanent ne répète la phase de préparation ou l’état des engagements déjà visible dans l’en-tête. L’en-tête aligne sur ordinateur le nom de la compétition à gauche et les badges de niveau et d’état à droite, avec les dates puis la ville immédiatement sous le nom côté Club, et les dates puis le site dans la vue organisateur ; les badges partagent exactement les mêmes hauteur, padding et taille de texte. Sur mobile, ils repassent sous la date et le lieu. Une compétition annulée reçoit sous cet en-tête une alerte rouge explicite, visible dans les vues organisateur et club. Dans les résumés Général organisateur et club, une ligne unique « Lieu » réunit dans cet ordre l’équipement ou le site, l’adresse puis la ville, séparés par des points médians et sans répéter deux valeurs identiques ; si l’adresse saisie contient déjà la ville, celle-ci n’est pas ajoutée une seconde fois. Une ligne unique « Bassin » associe ensuite la longueur, le nombre de lignes d’eau et le mode de chronométrage ; pour l’eau libre, la ligne équivalente « Plan d’eau » associe le type de plan d’eau et le chronométrage. Les champs Région et Régions invitées sont entièrement masqués en lecture comme en modification pour une compétition nationale ou internationale. La présentation publique occupe, lorsqu’elle est renseignée, un encadré dédié au-dessus des données : « Présentation publique » pour l’organisateur et « Informations de l’organisateur » pour le club. Le club voit également l’organisateur, son email public et la limite d’épreuves par nageur. Le lien compact « Payer sur HelloAsso », ou l’indication qu’il n’est pas renseigné, apparaît uniquement dans Récapitulatif lorsque des frais sont activés. Les actions organisateur Modifier/Supprimer ou Enregistrer/Annuler restent sur une même ligne tant que leurs libellés tiennent, chacune à la largeur de son contenu ; l’action principale reste turquoise et la suppression conserve son traitement d’alerte. Le retour est abrégé en « Retour » sur mobile tout en conservant son libellé accessible complet. Dans la fiche Club uniquement, la phrase compacte « Vous effectuez les engagements pour le club » affiche le seul code du club juste au-dessus du nom de la compétition ; elle disparaît dans les vues organisateur, régionale et nationale. Les onglets commencent directement sous cet en-tête ; seuls les retours ponctuels utiles de chargement, d’enregistrement ou d’erreur apparaissent lorsque nécessaire. La navigation compacte reste fixée sous l’en-tête principal pendant le défilement. Dans l’espace Club, quatre grandes étapes numérotées restent visibles sur la première ligne : Informations, Participants, Engagements et Récapitulatif. La rubrique Informations regroupe les sous-onglets Général, Programme et Documents ; Documents reste accessible avant l'ouverture comme après la fermeture des engagements. Une seconde ligne ne montre les sous-onglets de l’étape active que lorsqu’il en existe plusieurs ; Récapitulatif ne répète donc pas son bouton sous la navigation. Dans l’espace organisateur, la fiche n’est pas présentée comme un parcours obligatoire : les rubriques non numérotées « Paramétrage », « Statistiques », « GED » et « Diffusion » séparent la configuration, le suivi des engagements, les téléchargements et les courriels envoyés. L’email organisateur est également publié dans les informations pratiques du calendrier fédéral. La GED commence par les documents destinés aux clubs, présentés en liste compacte avec catégorie, format, taille et description. L'ajout multiple ouvre un formulaire dans la fiche ; sur mobile, ses champs et actions occupent toute la largeur. Les fichiers techniques proposent ensuite directement l’export TXT et le PDF du club choisi, sans exposer l’étape technique de génération. Les statistiques utilisent des indicateurs compacts puis des tableaux défilables par course et par club. Cette navigation ne repose ni sur un menu déroulant ni sur un défilement horizontal. Les intitulés « Programme » et « Courses individuelles » distinguent clairement la configuration des épreuves de l’engagement des nageurs.
...
Le portail utilise trois tailles explicites. La taille compacte, matérialisée par `.admin-button-compact`, mesure `36 px` de haut (`38 px` sur mobile) avec un padding horizontal de `10–12 px` ; elle est réservée aux retours, actions d’en-tête, barres d’outils et opérations courtes comme Modifier, Annuler ou Supprimer. La taille standard de `40 px` reste celle des formulaires. La taille large de `44 px` est réservée aux actions isolées qui nécessitent une forte présence, comme la connexion ou certaines confirmations. Un bouton ne prend toute la largeur sur mobile que s’il constitue l’unique action principale d’un formulaire ou si son libellé ne tient pas dans une rangée accessible. Dans les en-têtes, plusieurs actions courtes conservent leur largeur de contenu et passent à la ligne si nécessaire.

Pendant la soumission d'un formulaire, l'action principale conserve sa place mais affiche un indicateur discret et un libellé explicite, par exemple « Connexion en cours… » ou « Envoi en cours… ». Le bouton est désactivé pour empêcher un double envoi et le formulaire expose `aria-busy="true"` jusqu'à la fin de l'opération. L'animation respecte `prefers-reduced-motion`.
...
Les classes `.panel`, `.quick-card`, `.performance-link`, `.public-home-card`, `.public-home-portal` et `.admin-portal-card` partagent les mêmes principes : surface blanche, bordure claire, rayon de `8 px`, contenu structuré et ombre éventuelle.
...
Le calendrier public utilise une liste compacte groupée par mois. Toute création y est publiée immédiatement, quel que soit le type d’événement ; la fiche affiche « Les informations complémentaires seront publiées prochainement. » lorsque sa présentation n’est pas encore renseignée. Sur ordinateur, chaque ligne aligne date, nom/ville/type, périmètre et badge de statut. Le type d’événement est identifié à la fois par son libellé et par le liseré gauche : turquoise pour les compétitions piscine, bleu pour l’eau libre et ocre pour les formations, stages, réunions et autres événements non compétitifs. La région reste une information textuelle neutre. Sous 820 px, la ligne devient une carte à deux colonnes : le statut se place sous la date à gauche, tandis que niveau et région restent à droite. Les filtres Saison, Région, Niveau et Type sont regroupés dans un panneau blanc ; ils restent collants uniquement sur ordinateur afin de préserver la surface utile sur mobile. Le choix « National » du filtre Niveau inclut également les compétitions internationales, sans ajouter un choix distinct dans le filtre. Sur mobile, les quatre filtres occupent impérativement une grille de deux colonnes sur deux lignes : Saison/Région, puis Niveau/Type. Le filtre Période n’est pas affiché : pour la saison actuelle, une section « En cours et à venir » précède la section toujours visible « Depuis le début de la saison », qui regroupe les événements passés dans l’ordre chronologique. Un raccourci « Voir les événements passés » est placé près du compteur lorsque cette seconde section existe, afin de rester accessible même avec une longue liste d’événements à venir. Pour toute autre saison, tous les événements sont visibles immédiatement dans l’ordre de la saison sportive, de septembre à août. Le calendrier et les fiches publiques partagent l’intitulé d’en-tête sur une ligne « LivePalmes – Calendrier fédéral » ; les libellés des actions de navigation sont raccourcis sur mobile sans modifier leur destination. Les deux bandeaux turquoise privilégient une hauteur contenue : celui du calendrier réduit son logo fédéral et ses marges, tandis que celui de la fiche resserre ses espacements sans diminuer la taille du titre. Dans la fiche publique d’une compétition, l’en-tête remplace l’action « Accueil » par « Retour au calendrier ». Le contexte y est formulé explicitement, par exemple « Compétition piscine · Niveau national », plutôt que sous forme de deux mots isolés. L’email public de l’organisateur apparaît comme lien de contact dans la fiche, sans être utilisé par un envoi automatique. La colonne principale d’environ 58 % regroupe une carte « Informations pratiques » puis une carte « Documents » volontairement compacte ; le programme synthétique occupe les 42 % restants. Sous 820 px, les blocs sont empilés dans l’ordre Informations pratiques, Documents puis Programme. Au-delà de deux sessions, le programme devient une liste d’accordéons, avec le nom de session, sa date et ses horaires visibles ; avec quatre sessions ou plus, ils sont tous refermés à l’ouverture. Les accès sont regroupés en une courte colonne : « Accéder aux engagements LivePalmes » apparaît en premier, puis le groupe WhatsApp lorsqu’il existe. Ils partagent le même format de lien discret avec petite icône, sans fond ni grand bouton. La précision indiquant que les engagements sont réservés à un responsable de club est affichée juste en dessous, dans un texte secondaire discret. Dans le portail Club comme sur la fiche publique, le lien WhatsApp ouvre une fenêtre étroite contenant le QR code et un lien direct ; la bibliothèque QR est chargée uniquement au premier clic. Lorsque des résultats sont disponibles, la pastille « Résultats publiés » n’est pas répétée dans la fiche : sur ordinateur, les actions occupent une zone dédiée à droite des informations de l’en-tête et proposent directement la consultation en ligne ainsi que le protocole PDF lorsqu’il existe. Le même protocole est ajouté automatiquement en tête de la carte Documents, sauf si son URL y est déjà présente. Lorsqu’un fichier public de résultats structurés est associé, un bloc pleine largeur « Résultats de la compétition » apparaît sous la grille principale. Un sélecteur permet de choisir l’épreuve et le sexe ; les lignes affichent nageur, club, catégorie et temps sans reconstruire un classement absent de la source. Le nom d’un nageur individuel est un lien souligné vers sa fiche publique : une confirmation demande son ouverture dans un nouvel onglet. Les relais ne proposent pas de lien. Lorsqu’un même nageur possède plusieurs temps dans l’épreuve affichée, ils sont tous conservés ; une note discrète renvoie vers le protocole, qui fait foi pour le classement officiel. Sous 620 px, chaque temps devient une ligne à deux colonnes sans défilement horizontal. Sous 820 px, les actions du bandeau repassent sous les informations et, sur mobile, occupent chacune une ligne complète. Les dates de session utilisent le format français en toutes lettres.

Les événements sur plusieurs jours affichent une plage de dates compacte. Le résumé distingue simplement les événements à venir des événements passés. Une légende compacte, toujours visible à droite de ce résumé, rappelle les trois couleurs de type. Une action discrète « Réinitialiser » apparaît uniquement lorsqu’un filtre ou une saison différente est sélectionné. Le niveau « National / international » reflète explicitement le périmètre du filtre.

Dans les résultats structurés du calendrier, la colonne compacte « Repères » suit le temps. Elle affiche `PB` pour le meilleur temps personnel actuellement connu et, seulement en l’absence de `PB`, `SB` pour le meilleur temps de la saison sportive concernée. Le repère `PB` utilise un turquoise légèrement plus soutenu que le `SB`, volontairement plus neutre. Les deux calculs regroupent les bassins de 25 et 50 mètres ainsi que les chronométrages manuels et électroniques, sont recalculés à chaque génération, et sont expliqués par une légende discrète au-dessus du tableau. Chaque définition de la légende reste insécable : les deux tiennent sur une ligne lorsque la largeur le permet et passent proprement sur deux lignes sinon. Un filtre « Catégorie », initialisé sur « Toutes », est placé à côté du filtre « Épreuve » ; ses choix sont reconstruits à partir des catégories réellement présentes dans l’épreuve et le sexe sélectionnés, et il revient sur « Toutes » lors d’un changement d’épreuve. La catégorie est affichée au singulier sans répéter le sexe déjà présent dans le sélecteur d’épreuve ; les catégories d’âge Master sont libellées sous la forme « Master 40+ ». Sous `620 px`, les deux filtres restent sur une même ligne, puis la première ligne de chaque résultat réunit le nom, le temps et le repère ; le club et la catégorie occupent la seconde ligne.
...
### Surfaces du portail en mode sombre

Le thème sombre reste limité au portail. Les encarts du compte, le contexte du club, les filtres, les états vides et les tableaux utilisent une palette commune : fond `#101a22`, carte `#192630`, surface secondaire `#20323e`, texte `#e1ebef` et texte secondaire `#a8bbc5`. Les séparateurs restent discrets. Seul le logo fédéral conserve son support blanc. Les contenants des icônes reçoivent une teinte atténuée, sans ajouter de fond aux SVG de navigation. Les états succès, avertissement et erreur gardent leurs couleurs et libellés ; les lignes femmes/hommes conservent une distinction prune/bleu atténuée. Les règles sont conditionnées au thème sombre et ne changent ni les pages publiques ni les exports.

Les bandeaux d’accueil des espaces remplacent intégralement leur dégradé clair par une surface sombre, y compris la couche `background-image`. La carte nationale prioritaire utilise la surface secondaire sombre ; les surtitres restent turquoise et les libellés secondaires lisibles. Sous 760 px, les bandeaux gardent leur présentation compacte sur fond transparent.
