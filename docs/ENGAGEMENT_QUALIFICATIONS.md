...
Les exports nouveaux sont déclarés dans le lot `engagement-core`. Le workflow backend TEST actuel exige `main` ; cette branche ne modifie ni cette restriction ni les workflows de production. Aucun déploiement Firebase, migration ou écriture de données officielles n'est réalisé par cette proposition.
