...
Après validation de ce dispositif par le responsable du projet, le workflow `livepalmes-qualification-test-auto.yml` automatise cette séquence à chaque mise à jour de la branche : `bootstrap`, puis `engagement-core`, puis `publications`. Il appelle le workflow sécurisé existant avec le SHA exact du commit. Une étape en échec bloque les suivantes. Il ne peut appeler ni les lots email/schedulers, ni un projet de production. Le workflow manuel reste disponible pour les reprises ciblées.
