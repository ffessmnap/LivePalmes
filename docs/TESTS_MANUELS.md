...
- Vérifier que « Mes nageurs » est trié alphabétiquement avec le nom avant le prénom, qu’une licence absente est libellée « Licence à renseigner ».
...
- Sur un cache DTN chaud, verifier que la vue est immediate. Forcer ensuite un recalcul dans un environnement de test : la derniere vue doit rester affichee, le message doit annoncer le traitement en arriere-plan et un seul document `dtnQualificationJobs` doit etre cree par saison, sexe et empreinte.
- Dans le calendrier des engagements, changer region, niveau, statut et filtre « mes competitions » : aucun nouvel appel `listEngagementCompetitions` ne doit partir. Changer de saison doit en produire exactement un.
...
- Avant la premiere mise en service des agrégats, appeler `rebuildEngagementClubAggregates` uniquement dans un environnement controle, par lots de 10 clubs maximum, puis verifier `engagementClubRosters.generatedAt` et `engagementClubPeopleRosters.generatedAt`. Ne jamais lancer cette reconstruction depuis une session club.
