...
- Avec un compte disposant de `engagements.club.manage` et du droit de changement national de club, cliquer le code club du bandeau, rechercher un club par code et par nom, puis le sélectionner. Vérifier que l’espace Club affiche et modifie uniquement les données de ce club, que le bouton « Revenir à mon club » restaure immédiatement le club d’appartenance et qu’aucune recherche ne lance de lecture Firestore. Sur mobile, vérifier que « Changer de club » reste accessible depuis le menu du compte. Déconnecter puis reconnecter : le club d’appartenance doit être restauré. Avec un compte sans ce droit, la sélection d’un autre club doit être refusée côté Cloud Functions.
