...
function showFallback(blobUrl, pdfName) {
  ...
  if (fallback && blobUrl) {
    ...
    fallback.innerHTML = `
      ...
      <a class="ghost-button compact" href="${escapeHtml(blobUrl)}" download="${escapeHtml(pdfName || "resultat.pdf")}">Télécharger</a>
    `;
  }
}
...
async function init() {
  ...
  if (statusBox) statusBox.hidden = true;
  showFallback(blobUrl, result.pdfName || "resultat.pdf");
}
