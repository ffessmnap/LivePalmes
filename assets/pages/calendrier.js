(async function () {
  ...
  const nodes = { season: document.querySelector("#calendarSeason"), region: document.querySelector("#calendarRegion"), level: document.querySelector("#calendarLevel"), type: document.querySelector("#calendarType"), count: document.querySelector("#calendarCount"), historyLink: document.querySelector("#calendarHistoryLink"), resetFilters: document.querySelector("#calendarResetFilters"), list: document.querySelector("#calendarList") };
  let events = [];
  const currentSeasonEndYear = api.seasonEndYear();
  let selectedSeasonEndYear = currentSeasonEndYear;
  function dateShort(event) { const start = new Date(`${event.date}T12:00:00`); const end = new Date(`${event.endDate || event.date}T12:00:00`); const day = (date) => date.toLocaleDateString("fr-FR", { day:"2-digit" }); const dayMonth = (date) => date.toLocaleDateString("fr-FR", { day:"2-digit", month:"short" }); if (event.endDate && event.endDate !== event.date) { const sameMonth = start.getMonth() === end.getMonth() && start.getFullYear() === end.getFullYear(); return `<span>${sameMonth ? `${day(start)}–${dayMonth(end)}` : `${dayMonth(start)}–${dayMonth(end)}`}</span>`; } return `<span>${dayMonth(start)}</span>`; }
  ...
  function matchesLevel(eventLevel,selectedLevel) { if(!selectedLevel)return true;if(selectedLevel==="national")return eventLevel==="national"||eventLevel==="international";return eventLevel===selectedLevel; }
  function filteredEvents() { return events.filter((event) => !nodes.region.value || regionFilterValue(event) === nodes.region.value).filter((event) => matchesLevel(event.level,nodes.level.value)).filter((event) => !nodes.type.value || event.eventType === nodes.type.value); }
  function updateResetFilters() { if (nodes.resetFilters) nodes.resetFilters.hidden = selectedSeasonEndYear === currentSeasonEndYear && !nodes.region.value && !nodes.level.value && !nodes.type.value; }
  function eventHtml(event) { const state=displayStatus(event); return `<a class="calendar-event" data-event-type="${api.escapeHtml(event.eventType)}" href="competition.html?id=${encodeURIComponent(event.id)}"><time class="calendar-event-date" datetime="${api.escapeHtml(event.date)}">${dateShort(event)}</time><div class="calendar-event-main"><strong>${api.escapeHtml(event.name)}</strong><small><span class="calendar-event-type">${api.escapeHtml(api.TYPE_LABELS[event.eventType])}</span> · ${api.escapeHtml(event.city)}</small></div><div class="calendar-event-scope">${api.escapeHtml(api.LEVEL_LABELS[event.level])}${event.regionLabel ? `<br><small>${api.escapeHtml(event.regionLabel)}</small>` : ""}</div>${state ? `<span class="calendar-status" data-status="${state}">${api.escapeHtml(api.STATUS_LABELS[state])}</span>` : ""}</a>`; }
  ...
  function render() {
    ...
    nodes.count.textContent = `${current.length} à venir · ${past.length} passé${past.length > 1 ? "s" : ""}`;
  ...
  async function load(endYear) { nodes.list.innerHTML='<div class="calendar-empty">Chargement du calendrier…</div>'; try { const selectedRegion=nodes.region.value; const selectedRegionLabel=nodes.region.selectedOptions[0]?.textContent||selectedRegion; const data=await api.json(`seasons/${endYear}.json`); events=Array.isArray(data.events)?data.events:[]; const regionMap=new Map(events.filter(e=>e.regionId).map(e=>[regionFilterValue(e),regionFilterLabel(e)])); if(selectedRegion&&!regionMap.has(selectedRegion))regionMap.set(selectedRegion,selectedRegionLabel); const regions=[...regionMap.entries()].sort((a,b)=>a[1].localeCompare(b[1],"fr")); nodes.region.innerHTML='<option value="">Toutes</option>'+regions.map(([id,label])=>`<option value="${api.escapeHtml(id)}">${api.escapeHtml(label)}</option>`).join(""); nodes.region.value=selectedRegion; render(); } catch(error){ events=[]; nodes.count.textContent="Calendrier indisponible"; nodes.list.innerHTML=`<div class="calendar-error">${api.escapeHtml(error.message)}</div>`; } }
  [nodes.region,nodes.level,nodes.type].forEach(node=>node.addEventListener("change",render));
  nodes.resetFilters?.addEventListener("click", async () => { nodes.region.value=""; nodes.level.value=""; nodes.type.value=""; if (selectedSeasonEndYear !== currentSeasonEndYear && Array.from(nodes.season.options).some((option) => Number(option.value) === currentSeasonEndYear)) { nodes.season.value=String(currentSeasonEndYear); selectedSeasonEndYear=currentSeasonEndYear; await load(currentSeasonEndYear); return; } render(); });
