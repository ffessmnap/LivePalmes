(function attachLivePalmesPdfImport(global) {
  ...
  function fixPdfEncoding(value) {
    ...
    const mojibakePattern = /(?:Ã.|Â.|â€|â€™|â€œ|â€�|Å[“”œ’]|�)/;
    const mojibakeScore = (input) => (String(input || "").match(/(?:Ã.|Â.|â€|â€™|â€œ|â€�|Å[“”œ’]|�)/g) || []).length;
    ...
    const mojibakeByte = (char) => {
      const code = char.codePointAt(0);
      return windows1252Bytes.get(code) ?? (code & 255);
    };
    const repairMojibake = (input) => {
      let current = String(input || "");
      for (let pass = 0; pass < 3 && mojibakePattern.test(current); pass += 1) {
        const bytes = Uint8Array.from([...current].map(mojibakeByte));
        const repaired = new TextDecoder("utf-8", { fatal: false }).decode(bytes);
        if (!repaired || repaired.includes("�") || mojibakeScore(repaired) >= mojibakeScore(current)) break;
        current = repaired;
      }
      return current;
    };
    ...
    text = text.normalize("NFC")
      .replace(/C[¸̧]/g, "C")
      .replace(/c[¸̧]/g, "ç")
      .replace(/\bFRAN(?:\u00c1|\?|\uFFFD|\u25A1|\u2026|\.{3})OIS\b/g, "FRAN\u00c7OIS")
      .replace(/\bFran(?:\u00c1|\?|\uFFFD|\u25A1|\u2026|\.{3})ois\b/g, "Fran\u00e7ois")
      .replace(/\bFRAN[«‹]OIS\b/gi, "FRANÇOIS")
      .replace(/\bDOUY(?:…|\.{3}|�|□)RE\b/gi, "DOUYERE")
      .replace(/\bFRAN(?:C|…|\.{3}|�|□)OIS\b/gi, "FRANÇOIS")
      .replace(/\bST(?:\?|\uFFFD|□|…|\.{3})PHANE\b/g, "ST\u00c9PHANE")
      .replace(/\bSt(?:\?|\uFFFD|□|…|\.{3})Phane\b/g, "St\u00e9phane")
      .replace(/\bSt(?:\?|\uFFFD|□|…|\.{3})phane\b/g, "St\u00e9phane")
      .replace(/\bst(?:\?|\uFFFD|□|…|\.{3})phane\b/g, "st\u00e9phane")
      .replace(/\bTH(?:\?|\uFFFD|\u25A1|\u2026|\.{3})VENOT\b/g, "TH\u00c9VENOT")
  ...
  function normalizePdfUppercaseEToken(token) {
    ...
    if (/^TH\u00c9VENOT$/.test(text)) return text;
